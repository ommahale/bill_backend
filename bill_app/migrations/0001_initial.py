# Generated by Django 3.2 on 2023-04-10 05:18

from django.db import migrations, models
import django.db.models.deletion
import uuid


class Migration(migrations.Migration):

    initial = True

    dependencies = [
    ]

    operations = [
        migrations.CreateModel(
            name='Bill',
            fields=[
                ('uid', models.UUIDField(db_index=True, default=uuid.uuid4, editable=False, primary_key=True, serialize=False)),
                ('created_at', models.DateTimeField(auto_now_add=True)),
                ('updated_at', models.DateTimeField(auto_now=True)),
                ('bill_date', models.DateField()),
                ('amount', models.FloatField()),
                ('due_date', models.DateField()),
                ('incentive_due_date', models.DateField()),
                ('incentive_amount', models.FloatField()),
                ('status', models.CharField(default='unpaid', max_length=100)),
                ('units_consumed', models.FloatField()),
                ('bill_desc', models.CharField(max_length=100)),
                ('region', models.CharField(default='', max_length=50)),
                ('electrical_duty', models.FloatField(default=0)),
                ('has_fault', models.BooleanField(default=False)),
                ('penalty_amount', models.FloatField()),
                ('current_reading', models.CharField(max_length=100)),
                ('consumer_name', models.CharField(max_length=100)),
            ],
            options={
                'abstract': False,
            },
        ),
        migrations.CreateModel(
            name='BillUnit',
            fields=[
                ('uid', models.UUIDField(db_index=True, default=uuid.uuid4, editable=False, primary_key=True, serialize=False)),
                ('created_at', models.DateTimeField(auto_now_add=True)),
                ('updated_at', models.DateTimeField(auto_now=True)),
                ('unit_number', models.IntegerField()),
            ],
            options={
                'abstract': False,
            },
        ),
        migrations.CreateModel(
            name='Category',
            fields=[
                ('uid', models.UUIDField(db_index=True, default=uuid.uuid4, editable=False, primary_key=True, serialize=False)),
                ('created_at', models.DateTimeField(auto_now_add=True)),
                ('updated_at', models.DateTimeField(auto_now=True)),
                ('connection_category', models.CharField(max_length=100)),
                ('connection_type', models.CharField(max_length=100)),
            ],
            options={
                'abstract': False,
            },
        ),
        migrations.CreateModel(
            name='FaultBill',
            fields=[
                ('uid', models.UUIDField(db_index=True, default=uuid.uuid4, editable=False, primary_key=True, serialize=False)),
                ('created_at', models.DateTimeField(auto_now_add=True)),
                ('updated_at', models.DateTimeField(auto_now=True)),
                ('fault_reason', models.CharField(max_length=100)),
                ('bill', models.OneToOneField(on_delete=django.db.models.deletion.CASCADE, related_name='fault_bill', to='bill_app.bill')),
            ],
            options={
                'abstract': False,
            },
        ),
        migrations.CreateModel(
            name='Threshold',
            fields=[
                ('uid', models.UUIDField(db_index=True, default=uuid.uuid4, editable=False, primary_key=True, serialize=False)),
                ('created_at', models.DateTimeField(auto_now_add=True)),
                ('updated_at', models.DateTimeField(auto_now=True)),
                ('threshold', models.FloatField()),
            ],
            options={
                'abstract': False,
            },
        ),
        migrations.CreateModel(
            name='Voucher',
            fields=[
                ('created_at', models.DateTimeField(auto_now_add=True)),
                ('updated_at', models.DateTimeField(auto_now=True)),
                ('uid', models.UUIDField(default=uuid.uuid4, editable=False)),
                ('voucher_no', models.BigAutoField(primary_key=True, serialize=False)),
                ('amount', models.FloatField()),
                ('incentive_amount', models.FloatField()),
                ('unit', models.IntegerField()),
                ('voucher_desc', models.CharField(max_length=50)),
                ('voucher_date', models.DateField(auto_now_add=True)),
                ('is_paid', models.BooleanField(default=False)),
                ('last_bill_date', models.DateField()),
                ('bills', models.ManyToManyField(to='bill_app.Bill')),
                ('fault_bills', models.ManyToManyField(blank=True, to='bill_app.FaultBill')),
            ],
            options={
                'abstract': False,
            },
        ),
        migrations.CreateModel(
            name='RecieptFile',
            fields=[
                ('uid', models.UUIDField(db_index=True, default=uuid.uuid4, editable=False, primary_key=True, serialize=False)),
                ('created_at', models.DateTimeField(auto_now_add=True)),
                ('updated_at', models.DateTimeField(auto_now=True)),
                ('reciept_file', models.FileField(blank=True, null=True, upload_to='reciept_files/')),
                ('bill', models.OneToOneField(on_delete=django.db.models.deletion.CASCADE, related_name='reciept_file', to='bill_app.bill')),
            ],
            options={
                'abstract': False,
            },
        ),
        migrations.CreateModel(
            name='BillMeter',
            fields=[
                ('uid', models.UUIDField(db_index=True, default=uuid.uuid4, editable=False, primary_key=True, serialize=False)),
                ('created_at', models.DateTimeField(auto_now_add=True)),
                ('updated_at', models.DateTimeField(auto_now=True)),
                ('consumer_no', models.CharField(max_length=100)),
                ('billing_unit', models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, to='bill_app.billunit')),
            ],
            options={
                'abstract': False,
            },
        ),
        migrations.AddField(
            model_name='bill',
            name='bill_meter',
            field=models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, to='bill_app.billmeter'),
        ),
        migrations.AddField(
            model_name='bill',
            name='connection',
            field=models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, to='bill_app.category'),
        ),
    ]
